...
add_action('forminator_custom_form_submit_before_set_fields', function( $entry, $form_id ) {
    if ( $form_id === 'YOUR_FORM_ID' ) { // Replace YOUR_FORM_ID with your actual form ID
        $post_data = [
            'post_title'   => sanitize_text_field( $entry['your_title_field'] ),
            'post_content' => wp_kses_post( $entry['your_content_field'] ),
            'post_status'  => 'draft', // Set to 'publish' for immediate publishing
